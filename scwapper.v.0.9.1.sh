...
dirpath=/private/var/tmp/scwapper
mkdir $dirpath
chmod 777 $dirpath
appname=$1
appname=${appname//\ /\\ }
touch $dirpath/$appname.txt
#recursive function
...
echo "Usage: [Storyboard Number].[ViewController Number] <m> | d | q"
echo "m - load the controller modally "
echo "d - dismiss the VC that is displayed modally currently"
echo "q - quit"
echo "examples: 5.0, 11.12 m, d, ..."
echo ""
echo -e "\033[0m"

#Get controller to load and do it
read whattoload howtoload
if [ "$whattoload" = "q" ]; then
break;
fi;
